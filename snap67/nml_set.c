	...
	{																			\
		return (type ## _Set_Key) {												\
			...
			.hsh = hash_default(hashdata(k), hashlen(k), set->seed),			\
																				...
																				\
	void																		\
	name ## _copy(const type ## _Set *MAC_RESTRICT set,							\
					type ## _Set *MAC_RESTRICT dest)							\
	{																			\
		const size_t memsize = set->size * sizeof(type ## _Set_Buck);			\
		dest->bucks = realloc(dest->bucks, memsize);							\
		memcpy(dest->bucks, set->bucks, memsize);								\
																				\
		dest->size = set->size;													\
		dest->len = set->len;													\
		dest->seed = set->seed;													\
																				...
																				\
	void																		\
	name ## _union(const type ## _Set *MAC_RESTRICT set,						\
					type ## _Set *MAC_RESTRICT dest)							\
	{																			\
		...
		{																		\
			...
			if (buck->set)														\
			{																	\
				type ## _Set_Key hk = name ## _key(set, buck->hk.k);			\
				name ## _insert_buck(dest, hk, true);							\
			}																	\
		}																		\
	}																			\
																				\
	void																		\
	name ## _union_key(const type ## _Set *MAC_RESTRICT set,					\
						type ## _Set *MAC_RESTRICT dest)						\
	{																			\
		MAC_PREFETCH_CACHE(set->bucks, 0);										\
		MAC_PREFETCH_CACHE(dest->bucks, 1);										\
