...
static MAC_HOT inline void
parser_decl(Parser *parse, Ast_Node *node)
{
	switch (parse->curr.kind)
	{
		case TOK_TYPE:
			parser_decl_type(parse, (Decl_Node **)&node->node);
			break;

		case TOK_LET:
			parser_decl_let(parse, (Decl_Node **)&node->node);
			break;

		case TOK_FUN:
			parser_decl_fun(parse, (Decl_Node **)&node->node);
			break;
