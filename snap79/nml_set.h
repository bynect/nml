...
#define SET_DECL(name, type, key) 														\
	...
	MAC_CONST type ## _Set_Key name ## _key(const type ## _Set *set, const key k);		\
																						\
	void name ## _copy(const type ## _Set *MAC_RESTRICT set,							\
						type ## _Set *MAC_RESTRICT dest);								\
																						\
	void name ## _union(const type ## _Set *MAC_RESTRICT set,							\
						type ## _Set *MAC_RESTRICT dest);								\
																						\
	void name ## _union_key(const type ## _Set *MAC_RESTRICT set,						\
							type ## _Set *MAC_RESTRICT dest);							\
																						\
