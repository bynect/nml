...
dbg_dump_type_pretty(const Type type, uint32_t fun_depth, uint32_t tuple_depth)
{
	...
	else if (tag == TAG_NAME)
	{
		...
		fprintf(stdout, "%.*s", (uint32_t)name->name.len, name->name.str);
	}
	else if (tag == TAG_CONS)
	{
		Type_Cons *cons = TYPE_PTR(type);
		fprintf(stdout, "Var %.*s",
				(uint32_t)cons->name.len, cons->name.str);

		if (cons->cons != TYPE_NONE)
		{
			fprintf(stdout, " (");
			dbg_dump_type_pretty(cons->cons, fun_depth, tuple_depth);
			fputc(')', stdout);
		}
	}
	else
...
dbg_dump_ast_node(const Ast_Node *node, uint32_t depth, bool last)
{
	...
	switch (node->kind)
	{
		...
		case AST_TYPE:
			dbg_dump_ast_type((Ast_Type *)node, depth, last);
			break;
	}
}
...
void
dbg_dump_ast_type(const Ast_Type *node, uint32_t depth, bool last)
{
	dbg_dump_ast_pretty(depth, last, "Ast_Type `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);
	size_t i = 0;
	while (i < node->len - 1)
	{
		dbg_dump_ast_pretty(depth + 1, false, "Var `%.*s`\n",
							(uint32_t)node->vars[i].len, node->vars[i].str);
		dbg_dump_ast_pretty(depth + 2, true, "Type ");
		dbg_dump_type(node->types[i]);
		++i;
	}

	dbg_dump_ast_pretty(depth + 1, true, "Var `%.*s`\n",
						(uint32_t)node->vars[i].len, node->vars[i].str);
	dbg_dump_ast_pretty(depth + 2, true, "Type ");
	dbg_dump_type(node->types[i]);
}
...
{
#ifdef MAC_BIT64
	dbg_dump_hash_64(hsh);
#else
	dbg_dump_hash_32(hsh);
#endif
}
