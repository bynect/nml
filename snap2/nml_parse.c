...
static MAC_HOT Type parser_type(Parser *parse, bool var);
...
static MAC_HOT Type
parser_type_id(Parser *parse, bool var)
{
	if (parser_match(parse, TOK_LPAR))
	{
		Type type = parser_type(parse, var);
	...
	else
	{
		...
		Type_Name *type = arena_alloc(parse->arena, sizeof(Type_Name));
		type->name = name;
		return TYPE_TAG(TAG_NAME, type);
	}
}

static MAC_HOT Type
parser_type_fun(Parser *parse, bool var)
{
	bool paren = parser_match(parse, TOK_LPAR);
	uint16_t line = parse->curr.line;

	Type type = parser_type_id(parse, var);
	if (parser_match(parse, TOK_ARROW))
	{
		if (var && !paren)
		{
			if (!MAC_UNLIKELY(parse->flags & PARSE_SYNC))
			{
				...
				error_append(parse->err, parse->arena, STR_STATIC("Expected `(` before function type"),
							line, ERR_ERROR);
			}
		}
		...
		do
		{
			...
			pars[i++] = parser_type_id(parse, var);
		}
		while (parser_match(parse, TOK_ARROW));

		if (var)
		{
			parser_expect(parse, TOK_RPAR, STR_STATIC("Expected `)` after function type"));
		}
...
static MAC_HOT Type
parser_type_tuple(Parser *parse, bool var)
{
	Type type = parser_type_fun(parse, var);
	...
	if (parser_match(parse, TOK_STAR))
	{
		...
		do
		{
			...
			items[i++] = parser_type_fun(parse, var);
		}
...
static MAC_HOT Type
parser_type(Parser *parse, bool var)
{
	if (var && (parse->curr.kind != TOK_IDENT && parse->curr.kind != TOK_LPAR))
	{
		return TYPE_NONE;
	}
	return parser_type_tuple(parse, var);
}
...
parser_semi(Parser *parse, Ast_Node **node)
{
	...
	*node = ast_let_new(parse->arena, STR_STATIC("_"), *node, expr);
}
...
parser_let(Parser *parse, Ast_Node **node)
{
	...
	parse->scope = prev;
	*node = ast_let_new(parse->arena, name, value, expr);
}
...
parser_fun(Parser *parse, Ast_Node **node)
{
	...
	else if (parser_match(parse, TOK_EQ))
	{
		if  (--len == 0)
		{
			parser_error(parse, STR_STATIC("Expected `fun` parameters"));
	...
	parse->scope = prev;
	*node = ast_fun_new(parse->arena, name, pars, len, body, expr);
}
...
parser_top(Parser *parse, Ast_Node **node)
{
	...
	if (MAC_UNLIKELY(parser_match(parse, TOK_TYPE)))
	{
		Sized_Str name = parse->curr.str;
		parser_expect(parse, TOK_IDENT, STR_STATIC("Expected identifier"));
		parser_expect(parse, TOK_EQ, STR_STATIC("Expected `=` after identifier"));
		...
		Type *types = arena_lock(parse->arena, len * sizeof(Type));
		size_t i = 0;
		...
		do
		{
			if (MAC_UNLIKELY(i + 1 == len))
			{
				...
				types = arena_update(parse->arena, types, len * sizeof(Type));
			}

			vars[i] = parse->curr.str;
			parser_expect(parse, TOK_IDENT, STR_STATIC("Expected identifier"));

			for (size_t j = 1; j < i; ++j)
			{
				if (MAC_UNLIKELY(STR_EQUAL(vars[i], vars[j - 1])))
				{
					Sized_Str msg = format_str(parse->arena,
											STR_STATIC("Duplicated variant `{S}` in type `{S}`"),
											vars[i], name);
					parser_error(parse, msg);
				}
			}

			if (MAC_UNLIKELY(islower(vars[i].str[0])))
			{
				Sized_Str msg = format_str(parse->arena,
										STR_STATIC("Variant `{S}` of type `{S}` should be capitalized"),
										vars[i], name);
				parser_info(parse, msg);
			}

			types[i] = parser_type(parse, true);
			++i;
		}
		...
		types = arena_update(parse->arena, types, i * sizeof(Type));
		arena_unlock(parse->arena, types);

		*node = ast_type_new(parse->arena, name, vars, types, i);
	}
	else
	{
		parser_expr(parse, node);
