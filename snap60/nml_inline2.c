...
static MAC_HOT void
inliner_if(Inliner *inlin, Expr_Node **node)
{
...
static MAC_HOT void
inliner_match(Inliner *inlin, Expr_Node **node)
{
...
static MAC_HOT void
inliner_let(Inliner *inlin, Expr_Node **node)
{
...
static MAC_HOT void
inliner_fun(Inliner *inlin, Expr_Node **node)
{
...
static MAC_HOT void
inliner_lambda(Inliner *inlin, Expr_Node **node)
{
...
static MAC_HOT void
inliner_app(Inliner *inlin, Expr_Node **node)
{
			...
			for (size_t i = 0; i < len; ++i)
			{
				if (!str_set_member(&inlin->set, fun->pars[i]))
				{
					fun->body = expr_let_new(inlin->arena, fun->node.loc, TYPE_NONE,
											fun->pars[i], cast->args[i], fun->body);
				}
			}
...
static MAC_HOT void
inliner_tuple(Inliner *inlin, Expr_Node **node)
{
...
inliner_expr(Inliner *inlin, Expr_Node **node)
{
	...
	{
		case EXPR_IF:
			inliner_if(inlin, node);
			break;

		case EXPR_MATCH:
			inliner_match(inlin, node);
			break;

		case EXPR_LET:
			inliner_let(inlin, node);
			break;

		case EXPR_FUN:
			inliner_fun(inlin, node);
			break;

		case EXPR_LAMBDA:
			inliner_lambda(inlin, node);
			break;

		case EXPR_APP:
			inliner_app(inlin, node);
			break;
