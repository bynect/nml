...
{
	Sized_Str name = format_str(check->arena, STR_STATIC("T{u}"), check->tid++);
	return type_var_new(check->arena, name);
}
...
checker_apply_rec(Type_Checker *check, Type type, const Type_Tab *tab, bool *subst)
{
	...
	if (tag == TAG_FUN)
	{
		...
		{
			*subst = true;
			return type_fun_new(check->arena, par, ret);
		}
	}
	else if (tag == TAG_TUPLE)
	{
		...
		if (tmp)
		{
			...
			*subst = true;
			return type_tuple_new(check->arena, copy, tuple->len);
		}
	}
	else if (tag == TAG_VAR)
	{
...
{
	str_set_reset(&check->set1);
	checker_ftv_type(type, &check->set1);

	str_set_reset(&check->set2);
	checker_ftv_ctx(ctx, &check->set2);

	size_t len = check->set1.len;
	Sized_Str *vars = NULL;

	// NOTE: Set difference optimization
	for (size_t i = 0; i < check->set2.size; ++i)
	{
		Str_Set_Buck *buck = check->set2.bucks + i;
		if (buck->set)
		{
			if (str_set_remove_key(&check->set1, buck->hk))
			{
				--len;
			}
		}
	...
	if (len != 0)
	{
		...
		for (size_t i = 0, j = 0; i < check->set1.size; ++i)
		{
			Str_Set_Buck *buck = check->set1.bucks + i;
			if (buck->set)
			{
				vars[j++] = buck->hk.k;
			}
...
checker_infer_const(Lit_Value value)
{
	const Type types[] = {
#define TAG(name, _, kind)	[LIT_ ## name] = TYPE_ ## name,
#define TYPE
#define PRIM
#include "nml_tag.h"
#undef PRIM
...
checker_infer_patn(Type_Checker *check, Scheme_Tab *ctx, Patn_Node *node, Type_Tab *tab)
{
	...
	else if (node->kind == PATN_TUPLE)
	{
		...
		type = type_tuple_new(check->arena, items, tuple->len);
	}
	else
	{
		type = checker_infer_const(((Expr_Lit *)node)->value);
	}
...
checker_infer_match(Type_Checker *check, Scheme_Tab *ctx, Expr_Match *node, Type_Tab *tab)
{
	...
	if (!patn_match_check(node, check->arena, &set, check->err))
	{
		check->succ = false;
	}
	...
	{
		scheme_tab_copy(ctx, &tmp);
		Type patn = checker_infer_patn(check, &tmp, node->arms[i].patn, tab);
		Type expr = checker_infer_expr(check, &tmp, node->arms[i].expr, tab);

		checker_unify(check, value, patn, tab);
		checker_unify(check, type, expr, tab);
	}
...
checker_infer_fun(Type_Checker *check, Scheme_Tab *ctx, Expr_Fun *node, Type_Tab *tab)
{
	...
	for (size_t i = node->len; i > 0; --i)
	{
		...
		node->fun = type_fun_new(check->arena, par, node->fun);
	}
...
checker_infer_lambda(Type_Checker *check, Scheme_Tab *ctx, Expr_Lambda *node, Type_Tab *tab)
{
	...
	{
		Type par = checker_apply_type(check, pars[i - 1], &tmp1);
		fun = type_fun_new(check->arena, par, fun);
	}
	checker_compose(check, &tmp1, tab);
...
{
	if (node->fun->kind == EXPR_TUPLE || node->fun->kind == EXPR_LIT)
	{
	...
	for (size_t i = node->len; i > 0; --i)
	{
		...
		fun2 = type_fun_new(check->arena, par, fun2);
...
checker_infer_tuple(Type_Checker *check, Scheme_Tab *ctx, Expr_Tuple *node, Type_Tab *tab)
{
	...
	scheme_tab_free(&scope);
	return type_tuple_new(check->arena, items, node->len);
}
...
checker_infer_expr(Type_Checker *check, Scheme_Tab *ctx, Expr_Node *node, Type_Tab *tab)
{
	switch (node->kind)
	{
		...
		case EXPR_LIT:
			node->type = checker_infer_const(((Expr_Lit *)node)->value);
			break;
	}
	return node->type;
}
...
checker_decl_fun(Type_Checker *check, Decl_Fun *node, Type_Tab *tab)
{
	Scheme_Tab scope;
	scheme_tab_init(&scope, check->seed);
	scheme_tab_copy(check->ctx, &scope);
...
checker_decl_data(Type_Checker *check, Decl_Data *node, Type_Tab *tab)
{
	...
	for (size_t i = 0; i < node->len; ++i)
	{
		if (str_set_unique(&check->set1, node->constrs[i].name) & SET_FAIL)
		{
			Sized_Str msg = format_str(check->arena,
										STR_STATIC("Constructor `{S}` defined multiple times"),
										node->constrs[i].name);
			checker_error(check, node->node.loc, msg);
		}
	}
...
checker_init(Type_Checker *check, Hash_Default seed, Arena *arena, const Source *src, Scheme_Tab *ctx)
{
	...
	str_set_init(&check->set1, seed);
	str_set_init(&check->set2, seed);
}

void
checker_free(Type_Checker *check)
{
	str_set_free(&check->set1);
	str_set_free(&check->set2);
}
