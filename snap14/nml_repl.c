...
static void
repl_eval(Arena *arena, Hash_Default seed, const char *src, size_t len)
{
	Parser parse;
	...
	Module mod;
	module_init(&mod, STR_STATIC("@repl"));
...
repl_eof(Arena *arena, Hash_Default seed)
{
	...
	repl_eval(arena, seed, src, off);
}

static void
repl_file(Arena *arena, Hash_Default seed, const char *name)
{
...
repl_run(Arena *arena, Hash_Default seed)
{
	...
	{
		repl_eval(arena, seed, src, strlen(src));
		repl_dump("# ");
	}
	putc('\n', stdout);
}
