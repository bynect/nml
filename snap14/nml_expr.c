...
MAC_HOT Expr_Node *
expr_if_new(Arena *arena, Location loc, Expr_Node *cond, Expr_Node *b_then, Expr_Node *b_else)
{
...
MAC_HOT Expr_Node *
expr_match_new(Arena *arena, Location loc, Expr_Node *value, struct Expr_Match_Arm *arms, size_t len)
{
...
MAC_HOT Expr_Node *
expr_let_new(Arena *arena, Location loc, Sized_Str name, Type hint, Expr_Node *value, Expr_Node *expr)
{
...
MAC_HOT Expr_Node *
expr_fun_new(Arena *arena, Location loc, Sized_Str name, Type hint, Sized_Str *pars,
...
MAC_HOT Expr_Node *
expr_unary_new(Arena *arena, Location loc, Token_Kind op, Expr_Node *lhs)
{
	Expr_Unary *node = expr_node_new(arena, loc, EXPR_UNARY);
	node->op = op;
	node->lhs = lhs;
	return (Expr_Node *)node;
}
...
MAC_HOT Expr_Node *
expr_binary_new(Arena *arena, Location loc, Token_Kind op, Expr_Node *rhs, Expr_Node *lhs)
{
