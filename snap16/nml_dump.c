...
{
	if ((tok->kind >= TOK_IDENT && tok->kind <= TOK_CHAR) || (tok->kind <= TOK_COM))
	{
		fprintf(stdout, "%s(%.*s)\n", toks[tok->kind],
...
dump_ast_node(const Ast_Node node, uint32_t depth, bool last)
{
	...
	{
		case AST_EXPR:
			dump_expr_node(node.node, depth, last);
			break;

		case AST_SIG:
			dump_sig_node(node.node, depth, last);
			break;
	}
}
...
dump_expr_node(const Expr_Node *node, uint32_t depth, bool last)
{
	...
	switch (node->kind)
	{
		...
		case EXPR_UNARY:
			dump_expr_unary((Expr_Unary *)node, depth, last);
			break;

		case EXPR_BINARY:
			dump_expr_binary((Expr_Binary *)node, depth, last);
			break;
		...
		case EXPR_CONST:
			dump_expr_const((Expr_Const *)node, depth, last);
			break;
	}
}
...
dump_expr_fun(const Expr_Fun *node, uint32_t depth, bool last)
{
	...
	dump_node_pretty(depth, last, "Expr_Fun `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);
...
void
dump_expr_unary(const Expr_Unary *node, uint32_t depth, bool last)
{
	dump_node_pretty(depth, last, "Expr_Unary %s\n", toks[node->op]);
	dump_node_pretty(depth + 1, false, "lhs\n");
	dump_expr_node(node->lhs, depth + 2, true);
}

void
dump_expr_binary(const Expr_Binary *node, uint32_t depth, bool last)
{
	dump_node_pretty(depth, last, "Expr_Binary %s\n", toks[node->op]);
	dump_node_pretty(depth + 1, false, "rhs\n");
	dump_expr_node(node->rhs, depth + 2, true);
...
void
dump_expr_const(const Expr_Const *node, uint32_t depth, bool last)
{
	dump_node_pretty(depth, last, "Expr_Const\n");
	dump_node_pretty(depth + 1, true, "");
	dump_const(node->value);
}

void
dump_sig_node(const Sig_Node *node, uint32_t depth, bool last)
{
	fprintf(stdout, "%*s├ Location ", depth * 2, "");
	dump_location(node->loc);
	fputc('\n', stdout);
	...
	{
		dump_sig_type((Sig_Type *)node, depth, last);
	}
}

void
dump_sig_type(const Sig_Type *node, uint32_t depth, bool last)
{
	dump_node_pretty(depth, last, "Sig_Type `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);
...
void
dump_ir_type(const Ir_Type *type)
{
	switch (type->kind)
	{
		case IR_VOID:
		...
		case IR_FUN:
			dump_ir_type(type->fun.ret);
			fputc('(', stdout);

			dump_ir_type(type->fun.args);
			for (size_t i = 1; i < type->fun.len; ++i)
			{
				fprintf(stdout, ", ");
				dump_ir_type(type->fun.args + i);
			}
			fputc('(', stdout);
			break;
		...
		case IR_TUPLE:
			fprintf(stdout, "{ ");
			dump_ir_type(type->tuple.items);
			for (size_t i = 1; i < type->tuple.len; ++i)
			{
				fprintf(stdout, ", ");
				dump_ir_type(type->tuple.items + i);
			}
			fprintf(stdout, " }");
			break;

		case IR_INT:
			fprintf(stdout, "int%u", type->width);
			break;

		case IR_FLT:
			fprintf(stdout, "flt%u", type->width);
			break;
	}
}

void
dump_ir_value(const Ir_Value *value)
{
	switch (value->kind)
	{
		case VAL_ERR:
		...
		case VAL_GLOB:
			dump_ir_type(&value->type);
			fprintf(stdout, " #%.*s", (uint32_t)value->name.len, value->name.str);
			break;

		case VAL_LOCL:
			dump_ir_type(&value->type);
			fprintf(stdout, " $%d", value->id);
			break;
...
dump_instr(const Instr *instr)
{
	switch (instr->kind)
	{
		case INSTR_INEG:
		case INSTR_FNEG:
			dump_instr_unary(instr->kind, instr->unary);
			break;

		case INSTR_LOAD:
			dump_instr_load(instr->load);
			break;

		case INSTR_STORE:
			dump_instr_store(instr->store);
			break;

		case INSTR_ALLOCA:
			dump_instr_alloca(instr->alloca);
			break;

		case INSTR_GEP:
			dump_instr_gep(instr->gep);
			break;
		...
		case INSTR_CONDBR:
			dump_instr_condbr(instr->condbr);
			break;
		...
		case INSTR_CALL:
			dump_instr_call(instr->call);
			break;

		default:
			dump_instr_binary(instr->kind, instr->binary);
			break;
	}
}

void
dump_instr_unary(Instr_Kind kind, Instr_Unary instr)
{
	dump_ir_value(&instr.yield);
	fprintf(stdout, " = %s ", instrs[kind]);

	dump_ir_value(&instr.lhs);
	fputc('\n', stdout);
}

void
dump_instr_binary(Instr_Kind kind, Instr_Binary instr)
{
	dump_ir_value(&instr.yield);
	fprintf(stdout, " = %s ", instrs[kind]);

	dump_ir_value(&instr.rhs);
	fprintf(stdout, ", ");
	dump_ir_value(&instr.lhs);
	fputc('\n', stdout);
}
...
void
dump_instr_load(Instr_Load instr)
{
	dump_ir_value(&instr.yield);
	fprintf(stdout, " = load ");
	...
	fprintf(stdout, ", ");
	dump_ir_value(&instr.addr);
	fputc('\n' , stdout);
}

void
dump_instr_store(Instr_Store instr)
{
	fprintf(stdout, "store ");

	dump_ir_value(&instr.addr);
	fprintf(stdout, ", ");
	dump_ir_value(&instr.value);
	fputc('\n' , stdout);
}

void
dump_instr_alloca(Instr_Alloca instr)
{
	dump_ir_value(&instr.yield);
...
void
dump_instr_gep(Instr_Gep instr)
{
	dump_ir_value(&instr.yield);
	fprintf(stdout, " = gep ");
	dump_ir_type(&instr.type);
...
{
	fprintf(stdout, "br ");
	dump_ir_value(&instr.labl);
	fputc('\n', stdout);
}

void
dump_instr_condbr(Instr_Condbr instr)
{
	fprintf(stdout, "condbr ");
	dump_ir_value(&instr.cond);

	fprintf(stdout, ", ");
	dump_ir_value(&instr.b_then);

	fprintf(stdout, ", ");
	dump_ir_value(&instr.b_else);
	fputc('\n', stdout);
}
...
{
	fprintf(stdout, "ret");
	if (instr.value.type.kind != IR_VOID)
	{
		fputc(' ', stdout);
		dump_ir_value(&instr.value);
	}
...
void
dump_instr_call(Instr_Call instr)
{
	if (instr.yield.type.kind != IR_VOID)
	{
		dump_ir_value(&instr.yield);
		fprintf(stdout, " = ");
	}
	...
	fprintf(stdout, "call ");
	dump_ir_value(&instr.fun);
	fputc('(', stdout);

	dump_ir_value(&instr.args[0]);
	for (size_t i = 1; i < instr.len; ++i)
	{
		fprintf(stdout, ", ");
		dump_ir_value(&instr.args[i]);
	}
	fprintf(stdout, ")\n");
}
