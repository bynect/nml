...
static void
repl_eval(Arena *arena, Hash_Default seed, const Sized_Str name, const Sized_Str src)
{
	Parser parse;
	...
	if (parser_parse(&parse, &ast, &err))
	{
		...
		Type_Checker check;
		checker_init(&check, arena, seed, &ctx);
	...
	if (succ)
	{
		...
		//Compiler comp;
		//compiler_init(&comp, arena, &mod, seed);
...
repl_eof(Arena *arena, Hash_Default seed)
{
	...
	repl_eval(arena, seed, STR_STATIC("@repl"), STR_WLEN(src, off));
}
...
static inline void
repl_fmt(Arena *arena, const char *name)
{
	Sized_Str src = repl_file(arena, name);
...
repl_run(Arena *arena, Hash_Default seed)
{
	...
	{
		repl_eval(arena, seed, STR_STATIC("@repl"), STR_STRLEN(src));
		repl_dump("# ");
	}
	putc('\n', stdout);
}
...
repl_seed(void)
{
	...
	if (file == NULL || fread(&seed, sizeof(Hash_Default), 1, file) != sizeof(Hash_Default))
	{
		seed = (long)&file;
		return seed;
	}
...
main(int argc, const char **argv)
{
	...
	else if (argc == 3 && !strcmp(argv[1], "--file"))
	{
		...
		repl_eval(&arena, seed, STR_STRLEN(argv[2]), src);
	}
	else if (argc == 1)
	{
		repl_run(&arena, seed);
