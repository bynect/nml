...
static MAC_HOT void
checker_compose(Type_Checker *check, const Type_Tab *restrict s1, Type_Tab *restrict s2)
{
...
checker_infer_fun(Type_Checker *check, Scheme_Tab *ctx, Expr_Fun *node, Type_Tab *tab)
{
	...
	checker_unify(check, node->rec, node->fun, &tmp2);
	if (node->hint != TYPE_NONE)
	{
		checker_unify(check, node->hint, node->fun, &tmp2);
	}

	if (!STR_EQUAL(node->name, STR_STATIC("@anon")))
	{
		scheme_tab_remove(ctx, node->name);
...
static MAC_INLINE inline void
checker_sig(Type_Checker *check, Decl_Node *node, Type_Tab *tab)
{
	if (node->kind == DECL_TYPE)
	{
		Decl_Type *sig = (Decl_Type *)node;
		sig->type = checker_apply_type(check, sig->type, tab);
...
checker_infer(Type_Checker *check, Ast_Top *ast, Error_List *err)
{
	...
	{
		if (MAC_UNLIKELY(ast->items[i].kind == AST_DECL))
		{
		...
		else
		{
			...
			checker_apply_expr(check, node, &tab);
			if (MAC_UNLIKELY(!check->succ))
			{
				check->succ = true;
				succ = false;
			...
			if (node->kind == EXPR_FUN)
			{
				...
				{
					scheme_tab_remove(check->ctx, fun->name);
