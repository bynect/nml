...
static MAC_INLINE inline void parser_sig(Parser *parse, Decl_Node **node);
...
static MAC_HOT inline void parser_top_let(Parser *parse, Expr_Node **node, bool top);
static MAC_HOT inline void parser_top_fun(Parser *parse, Expr_Node **node, bool top);
static MAC_HOT inline void parser_top(Parser *parse, Ast_Node *node);
...
static MAC_INLINE inline void
parser_sig(Parser *parse, Decl_Node **node)
{
...
parser_expr_btick(Parser *parse, Expr_Node **node)
{
	...
	Expr_Node **args = arena_alloc(parse->arena, 2 * sizeof(Ast_Node *));
	args[0] = *node;
...
{
	parser_top_let(parse, node, false);
}

static MAC_HOT void
parser_expr_fun(Parser *parse, Expr_Node **node)
{
	parser_top_fun(parse, node, false);
}
...
static MAC_HOT inline void
parser_top_let(Parser *parse, Expr_Node **node, bool top)
{
	Location loc = parse->prev.loc;
	...
	Expr_Node *expr = NULL;
	if (MAC_LIKELY(!top) || parser_check(parse, TOK_IN))
	{
...
static MAC_HOT inline void
parser_top_fun(Parser *parse, Expr_Node **node, bool top)
{
	Location loc = parse->prev.loc;
	size_t len = 8;
	Sized_Str *pars = arena_lock(parse->arena, len * sizeof(Sized_Str));
	...
	if (!anon)
	{
		if (!top || parser_check(parse, TOK_IN))
		{
...
{
	switch (parse->curr.kind)
	{
		case TOK_TYPE:
			node->kind = AST_DECL;
			parser_sig(parse, (Decl_Node **)&node->node);
			parser_sync(parse);
			break;
		...
		case TOK_LET:
			...
			parser_top_let(parse, (Expr_Node **)&node->node, true);
			parser_sync(parse);
			break;
		...
		case TOK_FUN:
			...
			parser_top_fun(parse, (Expr_Node **)&node->node, true);
			parser_sync(parse);
			break;
