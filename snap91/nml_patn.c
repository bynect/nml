...
Patn_Node *
patn_lit_new(Arena *arena, Location loc, Lit_Value value)
{
	Patn_Lit *node = PATN_NEW(Patn_Lit, PATN_LIT, loc);
	node->value = value;
	return (Patn_Node *)node;
}
...
patn_refutable(const Patn_Node *node)
{
	...
	else if (node->kind == PATN_LIT)
	{
		return MAC_CAST(Patn_Lit *, node)->value.kind != LIT_UNIT;
	}
	return false;
}
...
patn_match_check(const Expr_Match *node, Arena *arena, Str_Set *set, Error_List *err)
{
	...
	if (succ && !wildcard)
	{
		...
		if (node->value->type == TYPE_CHAR)
		{
			...
			for (size_t i = 0; i < node->len; ++i)
			{
				if (node->arms[i].patn->kind == PATN_LIT)
				{
					cases[(uint8_t)MAC_CAST(Patn_Lit *, node->arms[i].patn)->value.v_char] = true;
				}
			}
		...
		else if (node->value->type == TYPE_BOOL)
		{
			...
			for (size_t i = 0; i < node->len; ++i)
			{
				if (node->arms[i].patn->kind == PATN_LIT)
				{
					cases[MAC_CAST(Patn_Lit *, node->arms[i].patn)->value.v_bool] = true;
				}
			}
