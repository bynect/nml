...
static MAC_INLINE inline void parser_const(Parser *parse, Lit_Value *value);
...
static MAC_INLINE inline void
parser_const_int(Parser *parse, Lit_Value *value)
{
	...
	const_init(value, LIT_INT, &c_int);
}

static MAC_INLINE inline void
parser_const_float(Parser *parse, Lit_Value *value)
{
	const double c_float = str_to_float(parse->prev.str);
	const_init(value, LIT_FLOAT, &c_float);
}

static MAC_HOT void
parser_const_chars(Parser *parse, Lit_Value *value)
{
	...
	if (parse->prev.kind == TOK_CHAR)
	{
		...
		const_init(value, LIT_CHAR, &c_char);
	}
	else
	{
		const Sized_Str c_str = STR_WLEN(buf, buflen);
		const_init(value, LIT_STR, &c_str);
	}
}

static MAC_HOT void
parser_const(Parser *parse, Lit_Value *value)
{
	...
	switch (parse->prev.kind)
	{
		...
		case TOK_TRUE:
			const_init(value, LIT_BOOL, &c_true);
			break;

		case TOK_FALSE:
			const_init(value, LIT_BOOL, &c_false);
			break;
...
parser_expr_group(Parser *parse, Expr_Node **node)
{
	...
	{
		*node = expr_const_new(parse->arena, loc, const_new(LIT_UNIT, NULL));
		return;
	}
...
parser_patn_base(Parser *parse, Patn_Node **node)
{
	if (parser_match(parse, TOK_LPAR))
	{
		...
		if (parser_match(parse, TOK_RPAR))
		{
			*node = patn_const_new(parse->arena, loc, const_new(LIT_UNIT, NULL));
		}
		else if (parser_check_oper(parse))
		{
			Sized_Str ident = parse->curr.str;
