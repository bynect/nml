...
Patn_Node *
patn_const_new(Arena *arena, Location loc, Lit_Value value)
{
...
patn_refutable(const Patn_Node *node)
{
	...
	{
		return MAC_CAST(Patn_Const *, node)->value.kind != LIT_UNIT;
	}
	return false;
}
	...
	if (succ && !wildcard)
	{
		...
		if (node->value->type == TYPE_CHAR)
		{
			...
			for (size_t i = 0; i < node->len; ++i)
			{
				...
				{
					cases[(uint8_t)MAC_CAST(Patn_Const *, node->arms[i].patn)->value.v_char] = true;
				}
			}
		...
		else if (node->value->type == TYPE_BOOL)
		{
			...
			for (size_t i = 0; i < node->len; ++i)
			{
				if (node->arms[i].patn->kind == PATN_CONST)
				{
					cases[MAC_CAST(Patn_Const *, node->arms[i].patn)->value.v_bool] = true;
				}
			}
