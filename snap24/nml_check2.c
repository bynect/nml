...
static MAC_HOT void
checker_freevars(Type type, Str_Buf *set)
{
	...
	else if (tag == TAG_VAR)
	{
		...
		str_buf_push(set, var->var);
	}
}

static MAC_HOT void
checker_freevars_scheme(Type_Scheme scheme, Str_Buf *set)
{
	for (size_t i = 0; i < scheme.len; ++i)
	{
		str_buf_push(set, scheme.vars[i]);
	}
}

static MAC_HOT void
checker_freevars_ctx(const Scheme_Tab *ctx, Str_Buf *set)
{
...
checker_generalize(Type_Checker *check, Type type, const Scheme_Tab *ctx)
{
	...
	str_buf_iter(&set1, str_buf_iter_dump, NULL);
	str_buf_iter(&set2, str_buf_iter_dump, NULL);
	...
	{
		if (!str_buf_member(&set2, set1.buf[i]))
		{
			str_buf_push(&diff, set1.buf[i]);
		}
	}
	...
	if (len != 0)
	{
		...
		memcpy(vars, diff.buf, len * sizeof(Sized_Str));
	}
...
checker_infer_patn(Type_Checker *check, Scheme_Tab *ctx, Patn_Node *node, Type_Tab *tab)
{
	...
	if (node->kind == PATN_IDENT)
	{
		...
		if (!STR_EQUAL(ident->name, STR_STATIC("_")))
		{
			...
			scheme_tab_insert(ctx, ident->name, checker_generalize(check, type, ctx));
		}
		else
		{
			type = TYPE_ANY;
...
checker_infer_match(Type_Checker *check, Scheme_Tab *ctx, Expr_Match *node, Type_Tab *tab)
{
	...
	for (size_t i = 0; i < node->len; ++i)
	{
		...
		checker_infer_patn(check, &tmp, node->arms[i].patn, tab);
		checker_unify(check, node->arms[i].patn->type, value, tab);

		checker_infer_expr(check, &tmp, node->arms[i].expr, tab);
		checker_unify(check, node->arms[i].expr->type, type, tab);
