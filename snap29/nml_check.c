...
checker_var_new(Type_Checker *check)
{
	...
	var->var = format_str(check->arena, STR_STATIC("@{u}"), check->tid++);
...
static inline void
checker_hint(Type_Checker *check, Location loc, const Scheme_Tab *ctx, Type_Scheme hint, Type type)
{
	Type_Scheme scheme = checker_generalize(check, type, ctx);
	if (hint.len > scheme.len)
	{
		Sized_Str msg = format_str(check->arena, STR_STATIC("Hint `{s}` is more general than `{s}`"),
									hint, scheme);
...
checker_subst_expr(Type_Checker *check, Expr_Node *node, const Type_Tab *tab)
{
	...
	switch (node->kind)
	{
		...
		case EXPR_LET:
			...
			if (MAC_CAST(Expr_Let *, node)->hint.type != TYPE_NONE)
			{
				checker_hint(check, node->loc, check->ctx,
							MAC_CAST(Expr_Let *, node)->hint,
							MAC_CAST(Expr_Let *, node)->value->type);
			}
			break;
		...
		case EXPR_FUN:
			...
			if (MAC_CAST(Expr_Fun *, node)->hint.type != TYPE_NONE)
			{
				checker_hint(check, node->loc, check->ctx,
							MAC_CAST(Expr_Fun *, node)->hint,
							MAC_CAST(Expr_Fun *, node)->fun);
			}
			break;
...
checker_infer_match(Type_Checker *check, Scheme_Tab *ctx, Expr_Match *node, Type_Tab *tab)
{
	...
	if (!pattern_match_check(node, check->arena, check->seed, check->err))
	{
		check->succ = false;
	}
...
{
	Type value = checker_infer_expr(check, ctx, node->value, tab);
	if (node->hint.type != TYPE_NONE)
	{
		Type hint = checker_instantiate(check, node->hint);
		checker_unify(check, hint, value, tab);
	}
	...
	if (node->expr != NULL)
	{
		...
		scheme_tab_insert(ctx, node->name, SCHEME_EMPTY(value));
...
checker_infer_fun(Type_Checker *check, Scheme_Tab *ctx, Expr_Fun *node, Type_Tab *tab)
{
	...
	if (!anon)
	{
		...
		scheme_tab_insert(ctx, node->name, SCHEME_EMPTY(node->rec));
	}
	...
	{
		node->fun = checker_fun_new(check, pars[i - 1], node->fun);
	}

	node->fun = checker_subst(check, node->fun, tab);
	scheme_tab_free(&tmp);

	if (node->hint.type != TYPE_NONE)
	{
		Type hint = checker_instantiate(check, node->hint);
		checker_unify(check, hint, node->fun, tab);
	}
...
static MAC_INLINE inline void
checker_sig(Type_Checker *check, Sig_Node *node, Type_Tab *tab)
{
	if (node->kind == SIG_TYPE)
	{
		Sig_Type *sig = (Sig_Type *)node;
		sig->type = checker_subst(check, sig->type, tab);
		type_tab_insert(tab, sig->name, sig->type);
	}
}
...
checker_infer(Type_Checker *check, Ast_Top *ast, Error_List *err)
{
	...
	for (size_t i = 0; i < ast->len; ++i)
	{
		...
		{
			checker_sig(check, ast->items[i].node, &tab);
		}
		else
		{
