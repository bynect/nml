...
{
	Patn_As *node = patn_node_new(arena, loc, PATN_AS);
	node->name = name;
	node->patn = patn;
	return (Patn_Node *)node;
}
...
patn_check(const Patn_Node *node, Arena *arena, Str_Set *set, Error_List *err)
{
	...
	else if (node->kind == PATN_AS)
	{
		...
		if (!patn_check(as->patn, arena, set, err))
		{
			ret = false;
		}

		if (str_set_unique(set, as->name) & SET_FAIL)
		{
			Sized_Str msg = format_str(arena,
										STR_STATIC("Identifier `{S}` used multiple times in pattern"),
										as->name);
			error_append(err, arena, msg, as->node.loc, ERR_ERROR);
			ret = false;
		}
	}
	else if (node->kind == PATN_TUPLE)
	{
		...
		for (size_t i = 0; i < tuple->len; ++i)
		{
			if (!patn_check(tuple->items[i], arena, set, err))
			{
				ret = false;
			}
		}
...
MAC_HOT bool
patn_match_check(const Expr_Match *node, Arena *arena, Hash_Default seed, Error_List *err)
{
	bool succ = true;
	bool wildcard = false;
