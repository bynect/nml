...
static const Precedence precs[] = {
	...
	[TOK_CONSTR] = { PFN(constr),	NULL,			PREC_NONE,		ASSOC_NONE 	},
...
parser_decl_data(Parser *parse, Decl_Node **node)
{
	...
	do
	{
		...
		parser_expect(parse, TOK_CONSTR, STR_STATIC("Expected `data` constructor name"));

		constrs[i].type = parser_check(parse, TOK_BAR) || parser_check_delim(parse) || parser_check(parse, TOK_EOF) ?
						TYPE_NONE : parser_type(parse);
		++i;
	}
	while (parser_match(parse, TOK_BAR));
...
parser_patn_base(Parser *parse, Patn_Node **node)
{
	...
	else if (parser_match(parse, TOK_CONSTR))
	{
