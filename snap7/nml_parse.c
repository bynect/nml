...
parser_patn_base(Parser *parse, Patn_Node **node)
{
	if (parser_match(parse, TOK_LPAR))
	{
		...
		{
			*node = patn_const_new(parse->arena, const_new(CONST_UNIT, NULL));
		}
		else if (parse->curr.kind >= TOK_PLUS && parse->curr.kind <= TOK_TILDE)
		{
			...
			*node = patn_ident_new(parse->arena, ident);
		}
		else
		{
	...
	{
		*node = patn_ident_new(parse->arena, parse->prev.str);
	}
	else if ((parse->curr.kind >= TOK_IDENT && parse->curr.kind <= TOK_FALSE))
	{
		...
		*node = patn_const_new(parse->arena, value);
	}
	else
	{
...
parser_patn_tuple(Parser *parse, Patn_Node **node)
{
	...
	if (parser_match(parse, TOK_COMMA))
	{
		...
		arena_unlock(parse->arena, items);
		*node = patn_tuple_new(parse->arena, items, i);
	}
}
...
parser_expr_group(Parser *parse, Ast_Node **node)
{
	if (parser_match(parse, TOK_RPAR))
	{
		*node = ast_const_new(parse->arena, const_new(CONST_UNIT, NULL));
		return;
	}
	else if (parse->curr.kind >= TOK_PLUS && parse->curr.kind <= TOK_TILDE)
	{
...
parser_expr_const(Parser *parse, Ast_Node **node)
{
	Const_Value value;
	parser_const(parse, &value);
	*node = ast_const_new(parse->arena, value);
}
