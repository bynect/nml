...
typedef enum {
#define TAG(name, _, kind)	VTAG_ ## name = VAL_MASK(VAL_BITOFF(6) + __COUNTER__),
#define VALUE
#include "nml_tag.h"
#undef VALUE
#undef TAG
...
#define VAL_UNIT			((Value)(VAL_NANMASK | VTAG_UNIT))
#define VAL_TRUE			((Value)(VAL_NANMASK | VTAG_TRUE))
#define VAL_FALSE			((Value)(VAL_NANMASK | VTAG_FALSE))
...
#define VAL_INT(int)		((Value)(VAL_NANMASK | VTAG_INT | (int)))
...
#define VAL_CHAR(chr)		((Value)(VAL_NANMASK | VTAG_CHAR | chr))
...
{
	return !VAL_BITSET(val, 63) && VAL_NANTAG(val, VTAG_INT);
}

static MAC_INLINE inline bool
value_is_char(Value val)
{
	return !VAL_BITSET(val, 63) && VAL_NANTAG(val, VTAG_CHAR);
}
