...
static inline void
dbg_dump_node_pretty(uint32_t depth, bool last, const char *fmt, ...)
{
...
void
dbg_dump_const(const Const_Value value)
{
	switch (value.kind)
	{
		case CONST_UNIT:
...
dbg_dump_ast_node(const Ast_Node *node, uint32_t depth, bool last)
{
	...
	{
		dbg_dump_node_pretty(depth, last, "`null`\n");
		return;
	}
	...
	switch (node->kind)
	{
		...
		case AST_MATCH:
			dbg_dump_ast_match((Ast_Match *)node, depth, last);
			break;
...
{
	dbg_dump_node_pretty(depth, last, "Ast_If\n");
	dbg_dump_node_pretty(depth + 1, false, "cond\n");
	dbg_dump_ast_node(node->cond, depth + 2, true);

	dbg_dump_node_pretty(depth + 1, false, "then\n");
	dbg_dump_ast_node(node->then, depth + 2, true);

	dbg_dump_node_pretty(depth + 1, true, "m_else\n");
	dbg_dump_ast_node(node->m_else, depth + 2, true);
}

void
dbg_dump_ast_match(const Ast_Match *node, uint32_t depth, bool last)
{
	dbg_dump_node_pretty(depth, last, "Ast_Match\n");
	dbg_dump_node_pretty(depth + 1, false, "value\n");
	dbg_dump_ast_node(node->value, depth + 2, true);

	size_t i = 0;
	while (i < node->len - 1)
	{
		dbg_dump_node_pretty(depth + 1, false, "arm %zu\n", i);
		dbg_dump_node_pretty(depth + 2, false, "pattern\n");
		dbg_dump_patn_node(node->arms[i].patn, depth + 3, true);

		dbg_dump_node_pretty(depth + 2, true, "expr\n");
		dbg_dump_ast_node(node->arms[i].expr, depth + 3, true);
		++i;
	}

	dbg_dump_node_pretty(depth + 1, true, "arm %zu\n", i);
	dbg_dump_node_pretty(depth + 2, false, "pattern\n");
	dbg_dump_patn_node(node->arms[i].patn, depth + 3, true);
...
dbg_dump_ast_let(const Ast_Let *node, uint32_t depth, bool last)
{
	...
	dbg_dump_node_pretty(depth, last, "Ast_Let `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);

	dbg_dump_node_pretty(depth + 1, false, "value\n");
	dbg_dump_ast_node(node->value, depth + 2, true);

	dbg_dump_node_pretty(depth + 1, true, "expr\n");
...
dbg_dump_ast_fun(const Ast_Fun *node, uint32_t depth, bool last)
{
	...
	dbg_dump_node_pretty(depth, last, "Ast_Fun `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);
	...
	if (node->len != 0)
	{
		...
		{
			dbg_dump_node_pretty(depth + 1, false, "param %u `%.*s`\n",
	...
	dbg_dump_node_pretty(depth + 1, false, "body\n");
...
{
	dbg_dump_node_pretty(depth, last, "Ast_App\n");
	dbg_dump_node_pretty(depth + 1, false, "fun\n");
	dbg_dump_ast_node(node->fun, depth + 2, true);

	size_t i = 0;
	while (i < node->len - 1)
	{
		dbg_dump_node_pretty(depth + 1, false, "arg %zu\n", i);
		dbg_dump_ast_node(node->args[i], depth + 2, true);
		++i;
	}

	dbg_dump_node_pretty(depth + 1, true, "arg %zu\n", i);
	dbg_dump_ast_node(node->args[i], depth + 2, true);
}
...
{
	dbg_dump_node_pretty(depth, last, "Ast_Ident `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);
}
...
{
	dbg_dump_node_pretty(depth, last, "Ast_Unary %s\n", toks[node->op]);
	dbg_dump_node_pretty(depth + 1, false, "lhs\n");
...
{
	dbg_dump_node_pretty(depth, last, "Ast_Binary %s\n", toks[node->op]);
	dbg_dump_node_pretty(depth + 1, false, "rhs\n");
	dbg_dump_ast_node(node->rhs, depth + 2, true);

	dbg_dump_node_pretty(depth + 1, true, "lhs\n");
	dbg_dump_ast_node(node->lhs, depth + 2, true);
}
...
{
	dbg_dump_node_pretty(depth, last, "Ast_Tuple `%zu items`\n", node->len);
	size_t i = 0;
	while (i < node->len - 1)
	{
		dbg_dump_node_pretty(depth + 1, false, "item %zu\n", i);
		dbg_dump_ast_node(node->items[i], depth + 2, true);
		++i;
	}

	dbg_dump_node_pretty(depth + 1, true, "item %zu\n", i);
	dbg_dump_ast_node(node->items[i], depth + 2, true);
}
...
{
	dbg_dump_node_pretty(depth, last, "Ast_Const\n");
	dbg_dump_node_pretty(depth + 1, true, "");
	dbg_dump_const(node->value);
}

void
dbg_dump_patn_node(const Patn_Node *node, uint32_t depth, bool last)
{
	fprintf(stdout, "%*s├ Type ", depth * 2, "");
	dbg_dump_type(node->type);
	...
	{
		case PATN_IDENT:
			dbg_dump_patn_ident((Patn_Ident *)node, depth, last);
			break;

		case PATN_TUPLE:
			dbg_dump_patn_tuple((Patn_Tuple *)node, depth, last);
			break;

		case PATN_CONST:
			dbg_dump_patn_const((Patn_Const *)node, depth, last);
			break;
	}
}

void
dbg_dump_patn_ident(const Patn_Ident *node, uint32_t depth, bool last)
{
	dbg_dump_node_pretty(depth, last, "Patn_Ident `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);
}

void
dbg_dump_patn_tuple(const Patn_Tuple *node, uint32_t depth, bool last)
{
	dbg_dump_node_pretty(depth, last, "Patn_Tuple `%zu items`\n", node->len);
	size_t i = 0;
	while (i < node->len - 1)
	{
		dbg_dump_node_pretty(depth + 1, false, "item %zu\n", i);
		dbg_dump_patn_node(node->items[i], depth + 2, true);
		++i;
	}
...
void
dbg_dump_patn_const(const Patn_Const *node, uint32_t depth, bool last)
{
	dbg_dump_node_pretty(depth, last, "Patn_Const\n");
	dbg_dump_node_pretty(depth + 1, true, "");
	dbg_dump_const(node->value);
}
