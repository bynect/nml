...
static MAC_HOT void
hoister_expr_if(Hoister *hoist, Expr_If *node)
{
...
static MAC_HOT void
hoister_expr_match(Hoister *hoist, Expr_Match *node)
{
...
static MAC_HOT void
hoister_expr_let(Hoister *hoist, Expr_Let *node)
{
	hoister_expr(hoist, node->value);
...
static MAC_HOT void
hoister_expr_fun(Hoister *hoist, Expr_Fun *node)
{
...
static MAC_HOT void
hoister_expr_lambda(Hoister *hoist, Expr_Lambda *node)
{
	hoister_expr(hoist, node->body);
...
static MAC_HOT void
hoister_expr_app(Hoister *hoist, Expr_App *node)
{
...
static MAC_HOT void
hoister_expr_cons(Hoister *hoist, Expr_Constr *node)
{
	if (node->value != NULL)
	{
		hoister_expr(hoist, node->value);
...
static MAC_HOT void
hoister_expr_tuple(Hoister *hoist, Expr_Tuple *node)
{
	for (size_t i = 0; i < node->len; ++i)
	{
...
hoister_expr(Hoister *hoist, Expr_Node *node)
{
	...
	{
		case EXPR_IF:
			hoister_expr_if(hoist, (Expr_If *)node);
			break;

		case EXPR_MATCH:
			hoister_expr_match(hoist, (Expr_Match *)node);
			break;

		case EXPR_LET:
			hoister_expr_let(hoist, (Expr_Let *)node);
			break;

		case EXPR_FUN:
			hoister_expr_fun(hoist, (Expr_Fun *)node);
			break;

		case EXPR_LAMBDA:
			hoister_expr_lambda(hoist, (Expr_Lambda *)node);
			break;

		case EXPR_APP:
			hoister_expr_app(hoist, (Expr_App *)node);
			break;
		...
		case EXPR_CONSTR:
			hoister_expr_cons(hoist, (Expr_Constr *)node);
			break;

		case EXPR_TUPLE:
			hoister_expr_tuple(hoist, (Expr_Tuple *)node);
			break;
	...
	switch (node->kind)
	{
		case DECL_LET:
			if (MAC_CAST(Decl_Let *, node)->value->kind == EXPR_LAMBDA)
			{
				...
				MAC_CAST(Decl_Fun *, node)->fun = fun->node.type;
			}
			else
			{
		...
		// TODO: Should data and type declaration be appended?
		case DECL_DATA:
			break;
