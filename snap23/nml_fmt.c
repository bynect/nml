...
{
	return format_int64(arena, buf, size, len, (int64_t)num, base);
}
...
{
	return format_uint64(arena, buf, size, len, (uint64_t)num, base);
}
			...
			uint32_t fun_depth, uint32_t tuple_depth)
{
	...
	else if (tag == TAG_VAR)
	{
		...
		return format_sized_str(arena, buf, size, len, var->var);
		//format_append(arena, buf, size, len + 2);
 		//(*buf)[len++] = 'V';
