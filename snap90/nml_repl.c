...
repl_eval(Arena *arena, Hash_Default seed, Scheme_Tab *ctx, Expr_Tab *def, const Source src)
{
	...
	if (succ)
	{
		...
		// TODO: Should this be done during compilation?
		//Hoister hoist;
	...
	fprintf(stdout, "Pipeline took %lf ms, terminating with %s\n",
			(end.tv_sec - start.tv_sec) * 1000 + (end.tv_nsec - start.tv_nsec) / 1e6,
			succ ? "success" : "error(s)");
...
main(int argc, const char **argv)
{
	...
	int exit = 0;
	const Hash_Default seed = repl_seed();
	...
	else if (argc == 3 && !strcmp(argv[1], "--file"))
	{
		...
		exit = repl_eval(&arena, seed, &ctx, &def, (Source) {STR_ZLEN(argv[2]), src});
	}
	else if (argc == 1)
	{
