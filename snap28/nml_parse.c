...
static MAC_HOT Type_Scheme parser_type_scheme(Parser *parse);
...
parser_type_scheme(Parser *parse)
{
	size_t len = 0;
	Sized_Str *vars = NULL;

	if (STR_EQUAL(parse->curr.str, STR_STATIC("forall")))
	{
		len = 4;
		...
		do
		{
			...
			parser_expect(parse, TOK_IDENT, STR_STATIC("Expected forall var"));
			vars[i++] = parse->prev.str;
		}
		while (parser_check(parse, TOK_IDENT));

		len = i;
		vars = arena_update(parse->arena, vars, len * sizeof(Type));
		arena_unlock(parse->arena, vars);

		parser_expect(parse, TOK_DOT, STR_STATIC("Expected `.` after forall"));
	}

	return (Type_Scheme) {
		.type = parser_type(parse),
		.vars = vars,
		.len = len,
	};
}
...
parser_expr_semi(Parser *parse, Expr_Node **node)
{
	...
	*node = expr_let_new(parse->arena, loc, STR_STATIC("_"), SCHEME_EMPTY(TYPE_NONE), *node, expr);
}
...
parser_top_let(Parser *parse, Expr_Node **node, bool top)
{
	...
	Type_Scheme hint = SCHEME_EMPTY(TYPE_NONE);
	if (parser_match(parse, TOK_COL))
	{
		hint = parser_type_scheme(parse);
	}
