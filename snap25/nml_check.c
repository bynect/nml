...
//#include "nml_check.h"
//
//static MAC_HOT void checker_annot_expr(Type_Checker *check, Expr_Node *node, Type_Tab *ctx);
//static MAC_HOT void checker_constr_expr(Type_Checker *check, Expr_Node *node, Constr_Buf *constr);
...
//		{
//			type_tab_insert(ctx, ident->name, node->type);
//		}
//	}
...
//	checker_annot_expr(check, node->cond, ctx);
//	checker_annot_expr(check, node->b_then, ctx);
//	checker_annot_expr(check, node->b_else, ctx);
//}
//
//static MAC_HOT inline void
...
//{
//	node->node.type = checker_var_new(check);
//	checker_annot_expr(check, node->value, ctx);
//
//	Str_Set set;
...
//		}
//
//		type_tab_union(ctx, &tmp);
//		checker_annot_patn(check, node->arms[i].patn, &tmp);
//
//		checker_annot_expr(check, node->arms[i].expr, &tmp);
//		type_tab_reset(&tmp);
//	}
//
//	type_tab_free(&tmp);
...
//	type_tab_insert(ctx, node->name, node->bind);
//
//	if (node->expr != NULL)
//	{
//		checker_annot_expr(check, node->expr, ctx);
//
//		if (shad)
//		{
//			type_tab_insert(ctx, node->name, type);
//		}
//		else
//		{
...
//		type_tab_insert(ctx, node->name, node->rec);
//	}
//
//	Type_Tab tmp;
//	type_tab_init(&tmp, check->seed);
...
//		type_tab_insert(&tmp, node->pars[i - 1], par);
//	}
//
//	checker_annot_expr(check, node->body, &tmp);
//	type_tab_free(&tmp);
//
//	if (node->expr != NULL)
//	{
...
//{
//	node->node.type = checker_var_new(check);
//	checker_annot_expr(check, node->fun, ctx);
//
//	if (node->fun->kind != EXPR_FUN && node->fun->kind != EXPR_LET &&
//		node->fun->kind != EXPR_IF && node->fun->kind != EXPR_MATCH &&
//		node->fun->kind != EXPR_IDENT)
//	{
...
//	for (size_t i = 0; i < node->len; ++i)
//	{
//		checker_annot_expr(check, node->args[i], ctx);
//	}
//}
//
//static MAC_HOT inline void
...
//{
//	node->node.type = checker_var_new(check);
//	checker_annot_expr(check, node->rhs, ctx);
//	checker_annot_expr(check, node->lhs, ctx);
//}
//
//static MAC_HOT inline void
...
//	for (size_t i = 0; i < node->len; ++i)
//	{
//		checker_annot_expr(check, node->items[i], ctx);
//	}
//}
//
//static MAC_HOT void
//checker_annot_expr(Type_Checker *check, Expr_Node *node, Type_Tab *ctx)
//{
//	switch (node->kind)
//	{
//		case EXPR_IF:
...
//	{
//		checker_constr_const(MAC_CAST(Patn_Const *, node)->value.kind, node->type, constr);
//	}
//}
//
//static MAC_HOT inline void
...
//{
//	checker_constr_expr(check, node->cond, constr);
//	checker_constr_set(constr, node->cond->type, TYPE_BOOL);
//
//	checker_constr_expr(check, node->b_then, constr);
//	checker_constr_set(constr, node->b_then->type, node->node.type);
//
//	checker_constr_expr(check, node->b_else, constr);
...
//{
//	checker_constr_expr(check, node->value, constr);
//
//	for (size_t i = 0; i < node->len; ++i)
//	{
//		checker_constr_patn(node->arms[i].patn, constr);
//		checker_constr_expr(check, node->arms[i].expr, constr);
//
...
//	else
//	{
//		checker_constr_expr(check, node->expr, constr);
...
//{
//	checker_constr_expr(check, node->body, constr);
...
//{
//	checker_constr_expr(check, node->fun, constr);
//
//	Type ret = node->node.type;
//	for (size_t i = node->len; i > 0; --i)
//	{
//		checker_constr_expr(check, node->args[i - 1], constr);
...
//	}
//
//	checker_constr_expr(check, node->rhs, constr);
//	checker_constr_expr(check, node->lhs, constr);
//
//	Type_Fun *fun = TYPE_PTR(type);
//	Type rhs = fun->par;
//
...
//	for (size_t i = 0; i < node->len; ++i)
//	{
//		checker_constr_expr(check, node->items[i], constr);
...
//
//static MAC_HOT void
//checker_constr_expr(Type_Checker *check, Expr_Node *node, Constr_Buf *constr)
//{
//	switch (node->kind)
//	{
//		case EXPR_IF:
...
//
//		case EXPR_CONST:
//			checker_constr_const(MAC_CAST(Expr_Const *, node)->value.kind, node->type, constr);
//			break;
//	}
//}
//
...
//
//static MAC_HOT void
//checker_subst_expr(Type_Checker *check, Subst_Tab *subst, Expr_Node *node)
//{
//	node->type = checker_subst_type(check, subst, node->type);
//	switch (node->kind)
//	{
//		case EXPR_IF:
//			checker_subst_expr(check, subst, MAC_CAST(Expr_If *, node)->cond);
//			checker_subst_expr(check, subst, MAC_CAST(Expr_If *, node)->b_then);
//			checker_subst_expr(check, subst, MAC_CAST(Expr_If *, node)->b_else);
//			break;
//
//		case EXPR_MATCH:
//			checker_subst_expr(check, subst, MAC_CAST(Expr_Match *, node)->value);
//
//			for (size_t i = 0; i < MAC_CAST(Expr_Match *, node)->len; ++i)
//			{
//				checker_subst_patn(check, subst, MAC_CAST(Expr_Match *, node)->arms[i].patn);
//				checker_subst_expr(check, subst, MAC_CAST(Expr_Match *, node)->arms[i].expr);
//			}
//			break;
//
//		case EXPR_LET:
//			checker_subst_expr(check, subst, MAC_CAST(Expr_Let *, node)->value);
//			if (MAC_CAST(Expr_Let *, node)->expr != NULL)
//			{
//				checker_subst_expr(check, subst, MAC_CAST(Expr_Let *, node)->expr);
//			}
//			break;
//
//		case EXPR_FUN:
//			checker_subst_expr(check, subst, MAC_CAST(Expr_Fun *, node)->body);
//			if (MAC_CAST(Expr_Fun *, node)->expr != NULL)
//			{
//				checker_subst_expr(check, subst, MAC_CAST(Expr_Fun *, node)->expr);
//			}
//			break;
//
//		case EXPR_APP:
//			checker_subst_expr(check, subst, MAC_CAST(Expr_App *, node)->fun);
//			for (size_t i = 0; i < MAC_CAST(Expr_App *, node)->len; ++i)
//			{
//				checker_subst_expr(check, subst, MAC_CAST(Expr_App *, node)->args[i]);
//			}
//			break;
//
//		case EXPR_IDENT:
...
//
//		case EXPR_BINARY:
//			checker_subst_expr(check, subst, MAC_CAST(Expr_Binary *, node)->rhs);
//			checker_subst_expr(check, subst, MAC_CAST(Expr_Binary *, node)->lhs);
//			break;
//
//		case EXPR_TUPLE:
//			for (size_t i = 0; i < MAC_CAST(Expr_Tuple *, node)->len; ++i)
//			{
//				checker_subst_expr(check, subst, MAC_CAST(Expr_Tuple *, node)->items[i]);
//			}
//			break;
//
//		case EXPR_CONST:
...
//		{
//			subst_tab_insert(subst, var, type);
//		}
//		return;
//	}
//
...
//
//void
//checker_init(Type_Checker *check, Hash_Default seed, Arena *arena, const Source *src, Type_Tab *ctx)
//{
//	check->seed = seed;
//	check->tid = 0;
//	check->arena = arena;
...
//		type_tab_insert(&check->env, sig->name, type);
//	}
//}
//
//MAC_HOT bool
...
//
//			checker_annot_expr(check, node, check->ctx);
...
//			}
//
//			checker_constr_expr(check, node, &constr);
//			if (MAC_UNLIKELY(!check->succ))
//			{
//				check->succ = true;
//				succ = false;
...
//				if (let->expr == NULL)
//				{
//					type_tab_insert(check->ctx, let->name, node->type);
//				}
//			}
//		}
//	}
