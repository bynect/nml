	...
	{																			\
		return (type ## _Tab_Key) {												\
			...
			.hsh = hash_default(hashdata(k), hashlen(k), tab->seed),			\
																				...
																				\
	void																		\
	name ## _union(const type ## _Tab *tab, type ## _Tab *dest)					\
	{																			\
...
#define TAB_SCHEMEDUMP(value)								\
	do														\
	{														\
		fprintf(stdout, "Type_Scheme:\n");					\
		dump_type(value.type);								\
															\
		fprintf(stdout, "vars:\n");							\
		for (size_t i = 0; i < value.len; ++i)				\
		{													\
			dump_sized_str(value.vars[i]);					\
		}													\
	}														\
	while (false)
...
TAB_IMPL(value_tab, Value, Sized_Str, Ir_Value,
		TAB_STRDATA, TAB_STRSIZE, STR_EQUAL,
		dump_sized_str, TAB_VALUEDUMP,
		malloc, free, 32, TAB_FACTOR, TAB_LOAD)

TAB_IMPL(scheme_tab, Scheme, Sized_Str, Type_Scheme,
		TAB_STRDATA, TAB_STRSIZE, STR_EQUAL,
		dump_sized_str, TAB_SCHEMEDUMP,
		malloc, free, 32, TAB_FACTOR, TAB_LOAD)

TAB_IMPL(str_tab, Str, Sized_Str, bool,
		TAB_STRDATA, TAB_STRSIZE, STR_EQUAL,
		dump_sized_str, TAB_INTDUMP,
		malloc, free, 16, TAB_FACTOR, TAB_LOAD)
