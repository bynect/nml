...
#define SET_DECL(name, type, key) 														\
	...
	MAC_CONST type ## _Set_Key name ## _key(const type ## _Set *set, const key k);		\
																						\
	void name ## _union(const type ## _Set *src, type ## _Set *dest, Arena *arena);		\
																						\
	void name ## _difference(const type ## _Set *set1, const type ## _Set *set2,		\
							type ## _Set *dest, Arena *arena);							\
																						\
	MAC_HOT bool name ## _lookup(const type ## _Set *set, key k);						\
																						\
	MAC_HOT bool name ## _lookup_key(const type ## _Set *set, type ## _Set_Key hk);		\
																						\
	MAC_HOT Set_Result name ## _insert(type ## _Set *set, Arena *arena, key k);			\
									 													\
	MAC_HOT Set_Result name ## _insert_key(type ## _Set *set, Arena *arena,				\
										type ## _Set_Key hk);							\
																						\
	MAC_HOT Set_Result name ## _unique(type ## _Set *set, Arena *arena, key k);			\
																						\
	MAC_HOT Set_Result name ## _unique_key(type ## _Set *set, Arena *arena,				\
									type ## _Set_Key hk);								\
																						\
