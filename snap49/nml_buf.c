...
#define BUF_IMPL(name, type, value, valuecmp, valuedump,					\
	...
	{																		\
		free(buf->buf);														\
																			...
																			\
	void																	\
	name ## _reserve(type ## _Buf *buf, size_t len)							\
	{																		\
		name ## _grow(buf, buf->len + len);									\
	}																		\
																			\
	void																	\
	name ## _append(const type ## _Buf *src, type ## _Buf *dest)			\
	{																		\
		...
		dest->len += src->len;												\
																			...
																			\
	MAC_HOT bool															\
	name ## _member(const type ## _Buf *buf, const value v)					\
	{																		\
		MAC_PREFETCH_CACHE(buf->buf, 0);									\
	...
	{																		\
		...
		{																	\
			fn(buf->buf + i, ctx);											\
																			...
																			\
	MAC_HOT void															\
	name ## _unique(type ## _Buf *buf, const value v)						\
	{																		\
		if (!name ## _member(buf, v))										\
		{																	\
			name ## _grow(buf, buf->len + 1);								\
			buf->buf[buf->len++] = v;										\
		}																	\
...
#define BUF_STRDUMP(value)				dump_sized_str(*value)

#define BUF_GLOBDUMP(value)				fputc('\n', stdout); dump_global(value)

//BUF_IMPL(constr_buf, Constr, Type_Constr, BUF_CONSTREQ, BUF_CONSTRDUMP,
//		malloc, realloc, free, 16, 2)

BUF_IMPL(glob_buf, Glob, Global, BUF_FALSE, BUF_GLOBDUMP,
		malloc, realloc, free, 16, 2)

BUF_IMPL(byte_buf, Byte, uint8_t, BUF_EQ, BUF_BYTEDUMP,
		malloc, realloc, free, 16, 2)

BUF_IMPL(str_buf, Str, Sized_Str,
		STR_EQUAL, BUF_STRDUMP,
		malloc, realloc, free, 16, 2)
