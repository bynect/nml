...
#define TAB_IMPL(name, type, key, value, hashdata, hashlen, keycmp, 			\
	...
	{																			\
		return (type ## _Tab_Key) {												\
			...
			.hsh = hash_default(hashdata(k), hashlen(k), tab->seed),			\
																				...
																				\
	void																		\
	name ## _union(const type ## _Tab *tab, type ## _Tab *dest, Arena *arena)	\
	{																			\
	...
	{																			\
		...
		return name ## _get_buck(tab, hk, v);									\
																				...
																				\
	MAC_HOT Tab_Result															\
	name ## _insert(type ## _Tab *tab, Arena *arena, key k, value v)			\
	{																			\
		...
		return res | name ## _insert_buck(tab, hk, v, false);					\
																				...
																				\
	MAC_HOT Tab_Result															\
	name ## _insert_key(type ## _Tab *tab, Arena *arena,						\
...
#define TAB_ASTDUMP(value)		dump_ast_node(value, 0, true)
#define TAB_SYMDUMP(value)		dump_symbol(&value)
#define TAB_VARDUMP(value) 		dump_variable(&value); fputc('\n', stdout)
...
TAB_IMPL(subst_tab, Subst, Type_Id, Type,
		TAB_INTDATA, TAB_INTSIZE, TAB_CMP,
		TAB_INTDUMP, dump_type,
		arena_alloc, 32, TAB_FACTOR, TAB_LOAD)

TAB_IMPL(ast_tab, Ast, Sized_Str, Ast_Node *,
		TAB_STRDATA, TAB_STRSIZE, STR_EQUAL,
		dump_sized_str, TAB_ASTDUMP,
		arena_alloc, 32, TAB_FACTOR, TAB_LOAD)
...
TAB_IMPL(var_tab, Var, Sized_Str, Variable,
		TAB_STRDATA, TAB_STRSIZE, STR_EQUAL,
		dump_sized_str, TAB_VARDUMP,
		arena_alloc, 32, TAB_FACTOR, TAB_LOAD)
