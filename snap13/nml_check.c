...
checker_subst_ast(Type_Checker *check, Subst_Tab *subst, Ast_Node *node)
{
	...
	switch (node->kind)
	{
		case AST_IF:
			...
			checker_subst_ast(check, subst, ((Ast_If *)node)->b_then);
			checker_subst_ast(check, subst, ((Ast_If *)node)->b_else);
			break;
...
checker_infer(Type_Checker *check, Ast_Top *ast, Error_List *err)
{
	...
	for (size_t i = 0; i < ast->len; ++i)
	{
		...
		//if (MAC_UNLIKELY(node->kind == AST_TYPE))
		//{
		//	checker_type(check, (Ast_Type *)node);
		//}
