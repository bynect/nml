...
static MAC_HOT void parser_group(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_semi(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_uline(Parser *parse, Expr_Node **node);

static MAC_HOT void parser_if(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_let(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_fun(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_app(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_ident(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_unary(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_binary(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_tuple(Parser *parse, Expr_Node **node);
static MAC_HOT void parser_const(Parser *parse, Expr_Node **node);

static MAC_HOT void parser_prec(Parser *parse, Expr_Node **node, Parser_Prec prec);
...
static MAC_HOT void
parser_group(Parser *parse, Expr_Node **node)
{
	if (parser_match(parse, TOK_RPAR))
	{
		*node = expr_const_new(parse->arena, CONST_UNIT, NULL);
		return;
	}
	else if (parse->curr.kind >= TOK_PLUS && parse->curr.kind <= TOK_TILDE)
	{
		...
		if (parser_match(parse, TOK_RPAR))
		{
			*node = expr_ident_new(parse->arena, ident);
			return;
		}
		parser_back(parse);
	}
...
static MAC_HOT void
parser_semi(Parser *parse, Expr_Node **node)
{
	Expr_Node *expr;
	parser_expr(parse, &expr);
	*node = expr_let_new(parse->arena, STR_STATIC("_"), TYPE_NONE, *node, expr);
}

static MAC_HOT void
parser_uline(Parser *parse, Expr_Node **node)
{
	MAC_UNUSED(node);
...
static MAC_HOT void
parser_if(Parser *parse, Expr_Node **node)
{
	Parser_Scope prev = parse->scope;
	parse->scope = SCOPE_LOCAL;
	...
	parse->scope = prev;
	*node = expr_if_new(parse->arena, cond, then, m_else);
}

static MAC_HOT void
parser_let(Parser *parse, Expr_Node **node)
{
	Sized_Str name = parse->curr.str;
	...
	parse->scope = prev;
	*node = expr_let_new(parse->arena, name, hint, value, expr);
}

static MAC_HOT void
parser_fun(Parser *parse, Expr_Node **node)
{
	size_t len = 8;
	...
	parse->scope = prev;
	*node = expr_fun_new(parse->arena, name, hint, pars, len, body, expr);
}

static MAC_HOT void
parser_app(Parser *parse, Expr_Node **node)
{
	if (parser_expr_literal(parse))
	{
		...
		*node = expr_app_new(parse->arena, *node, args, i);
	}
}

static MAC_INLINE inline void
parser_ident(Parser *parse, Expr_Node **node)
{
	*node = expr_ident_new(parse->arena, parse->prev.str);
}

static MAC_HOT void
parser_unary(Parser *parse, Expr_Node **node)
{
	Token_Kind op = parse->prev.kind;
	Expr_Node *lhs;
	parser_prec(parse, &lhs, PREC_UNARY);
	*node = expr_unary_new(parse->arena, op, lhs);
}

static MAC_HOT void
parser_binary(Parser *parse, Expr_Node **node)
{
	Token_Kind op = parse->prev.kind;
	Expr_Node *lhs;
	parser_prec(parse, &lhs, precs[op].prec + precs[op].assoc);
	*node = expr_binary_new(parse->arena, op, *node, lhs);
}

static MAC_HOT void
parser_tuple(Parser *parse, Expr_Node **node)
{
	...
	else
	{
		...
		*node = expr_tuple_new(parse->arena, items, 2);
	}
...
static MAC_INLINE inline void
parser_int(Parser *parse, Expr_Node **node)
{
	int64_t res;
	...
	*node = expr_const_new(parse->arena, CONST_INT, &c_int);
}

static MAC_INLINE inline void
parser_float(Parser *parse, Expr_Node **node)
{
	const double c_float = str_to_float(parse->prev.str);
	*node = expr_const_new(parse->arena, CONST_FLOAT, &c_float);
}

static MAC_HOT void
parser_chars(Parser *parse, Expr_Node **node)
{
	...
	if (parse->prev.kind == TOK_CHAR)
	{
		...
		*node = expr_const_new(parse->arena, CONST_CHAR, &c_char);
	}
	else
	{
		const Sized_Str c_str = STR_WLEN(buf, buflen);
		*node = expr_const_new(parse->arena, CONST_STR, &c_str);
	}
}

static MAC_HOT void
parser_const(Parser *parse, Expr_Node **node)
{
	...
	switch (parse->prev.kind)
	{
		...
		case TOK_TRUE:
			*node = expr_const_new(parse->arena, CONST_BOOL, &c_true);
			break;

		case TOK_FALSE:
			*node = expr_const_new(parse->arena, CONST_BOOL, &c_false);
			break;
...
static MAC_HOT void
parser_prec(Parser *parse, Expr_Node **node, Parser_Prec prec)
{
	parser_eat(parse);
...
static MAC_INLINE inline void
parser_top(Parser *parse, Expr_Node **node)
{
...
MAC_HOT bool
parser_parse(Parser *parse, Ast_Module *ast, Error_List *err)
{
	parse->err = err;
	...
	items = arena_update(parse->arena, items, i * sizeof(Expr_Node *));
	arena_unlock(parse->arena, items);
