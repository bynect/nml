...
{
	dbg_dump_ast_pretty(depth, last, "Ast_Match\n");
	...
	while (i < node->len - 1)
	{
		dbg_dump_ast_pretty(depth + 1, false, "arm %zu\n", i);
		dbg_dump_ast_pretty(depth + 2, false, "pattern\n");
		dbg_dump_ast_node(node->arms[i][0], depth + 3, true);

		dbg_dump_ast_pretty(depth + 2, true, "expr\n");
		dbg_dump_ast_node(node->arms[i][1], depth + 3, true);
		++i;
	}

	dbg_dump_ast_pretty(depth + 1, true, "arm %zu\n", i);
	dbg_dump_ast_pretty(depth + 2, false, "pattern\n");
	dbg_dump_ast_node(node->arms[i][0], depth + 3, true);
