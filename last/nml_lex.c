...
#define TOK(name, _, str, type)	{ STR_STATIC(#str),	TOK_ ## name },
#define KWD
#include "nml_tok.h"
#undef KWD
#undef TOK
...
static inline void
lexer_ident(Lexer *lex, Token *tok, Token_Kind kind)
{
...
lexer_num(Lexer *lex, Token *tok)
{
	...
	if (isalpha(lexer_peek(lex)) || lexer_peek(lex) == '_')
	{
...
lexer_next(Lexer *lex, Token *tok)
{
	...
	if (isalpha(c) || c == '_')
	{
		lexer_ident(lex, tok, isupper(c) ? TOK_UIDENT : TOK_IDENT);
		return true;
	}
