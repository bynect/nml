...
repl_eval(Arena *arena, Hash_Default seed, const Source src)
{
	...
	if (parser_parse(&parse, &ast, &err))
	{
		...
		type_tab_insert(&ctx, arena, STR_STATIC("neg"), type_ineg);
		type_tab_insert(&ctx, arena, STR_STATIC("fneg"), type_fneg);
		type_tab_insert(&ctx, arena, STR_STATIC("not"), type_not);
