...
dump_type_pretty(const Type type, uint32_t fun_depth, uint32_t tuple_depth)
{
	...
	else if (tag == TAG_CONSTR)
	{
		Type_Constr *constr = TYPE_PTR(type);
		fprintf(stdout, "%.*s", (uint32_t)constr->name.len, constr->name.str);

		for (size_t i = 0; i < constr->len; ++i)
		{
			fputc(' ', stdout);
			dump_type_pretty(constr->items[i], 0, 1);
		}
	}
	else
	{
...
dump_expr_node(const Expr_Node *node, uint32_t depth, bool last)
{
	...
	switch (node->kind)
	{
		...
		case EXPR_CONSTR:
			dump_expr_constr((Expr_Constr *)node, depth, last);
			break;
...
void
dump_expr_constr(const Expr_Constr *node, uint32_t depth, bool last)
{
	dump_node_pretty(depth, last, "Expr_Constr `%.*s`\n",
						(uint32_t)node->name.len, node->name.str);
...
dump_decl_data(const Decl_Data *node, uint32_t depth, bool last)
{
	...
	dump_node_pretty(depth + 1, false, "Type vars %u\n", node->varlen);
	for (size_t i = 1; i < node->varlen; ++i)
	{
		dump_node_pretty(depth + 2, false, "Var %u `%.*s`\n", i - 1,
						(uint32_t)node->vars[i - 1].len, node->vars[i - 1].str);
	}

	dump_node_pretty(depth + 2, true, "Var %u `%.*s`\n", node->varlen - 1,
					(uint32_t)node->vars[node->varlen - 1].len, node->vars[node->varlen - 1].str);
	...
	for (size_t i = 0; i < len; ++i)
	{
		...
		dump_node_pretty(depth + 2, true, "Type ");
		dump_type(node->constrs[i].type);
	}
	dump_node_pretty(depth + 1, true, "constr %u `%.*s`\n",
