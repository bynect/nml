...
parser_type_base(Parser *parse)
{
	...
	else if (parser_match(parse, TOK_UIDENT))
	{
		...
		parser_error(parse, STR_STATIC("Unknown type name"), false);
		return TYPE_NONE;
	}
	else
	{
...
parser_expr_constr(Parser *parse, Expr_Node **node)
{
	...
	if (!parser_check_delim(parse) && !parser_check(parse, TOK_EOF))
	{
...
parser_decl_data(Parser *parse, Decl_Node **node)
{
	...
	*node = decl_data_new(parse->arena, loc, name, scheme, constrs, i);
}
