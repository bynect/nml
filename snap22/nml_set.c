	...
	{																			\
		return (type ## _Set_Key) {												\
			...
			.hsh = hash_default(hashdata(k), hashlen(k), set->seed),			\
																				...
																				\
	void																		\
	name ## _union(const type ## _Set *src, type ## _Set *dest)					\
	{																			\
...
#define SET_INTDATA(key)		(void *)&key
#define SET_INTSIZE(key)		sizeof(key)
#define SET_INTDUMP(key)		fprintf(stdout, "Id: %u\n", key)
...
//SET_IMPL(var_set, Var, Type_Id,
//		SET_INTDATA, SET_INTSIZE, SET_EQ,
//		SET_INTDUMP,
//		malloc, free, 16, 2, SET_LOAD)
