...
folder_lit(Expr_Node *node)
{
	return node->kind == EXPR_LIT;
}

static MAC_INLINE inline Expr_Node *
folder_copy(Folder *fold, Expr_Lit *node)
{
	Expr_Lit *tmp = arena_alloc(fold->arena, sizeof(Expr_Lit));
	memcpy(tmp, node, sizeof(Expr_Lit));
	return (Expr_Node *)tmp;
}

static MAC_HOT void
folder_expr_if(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
	Expr_If *cast = (Expr_If *)*node;
	folder_expr(fold, &cast->cond, def);

	if (folder_lit(cast->cond))
	{
		if (MAC_CAST(Expr_Lit *, cast->cond)->value.v_bool)
		{
			*node = cast->b_then;
	...
	if (folder_lit(cast->b_then) && folder_lit(cast->b_else))
	{
		if (lit_equal(MAC_CAST(Expr_Lit *, cast->b_then)->value,
						MAC_CAST(Expr_Lit *, cast->b_else)->value))
		{
			*node = cast->b_then;
		}
	}
...
static MAC_HOT void
folder_expr_match(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
	...
	if (MAC_UNLIKELY(cast->arms[0].patn->kind == PATN_IDENT))
	{
		...
		if (folder_lit(cast->value) || cast->value->kind == EXPR_IDENT)
		{
	...
	else if (MAC_UNLIKELY(cast->arms[0].patn->kind == PATN_LIT &&
			MAC_CAST(Patn_Lit *, cast->arms[0].patn)->value.kind == LIT_UNIT))
	{
		*node = cast->arms[0].expr;
		folder_expr(fold, node, def);
		return;
	}
	else if (folder_lit(cast->value))
	{
		for (size_t i = 0; i < cast->len; ++i)
		{
			if (cast->arms[i].patn->kind == PATN_LIT &&
				lit_equal(MAC_CAST(Expr_Lit *, cast->value)->value, MAC_CAST(Patn_Lit *, cast->arms[i].patn)->value))
			{
	...
	else if (cast->value->kind == EXPR_TUPLE)
	{
		...
		{
			consts[i] = folder_lit(tuple->items[i]) || tuple->items[i]->kind == EXPR_IDENT;
		}
		...
		for (size_t i = 0; i < cast->len; ++i)
		{
			expr_tab_copy(def, &tmp);
			if (MAC_LIKELY(cast->arms[i].patn->kind == PATN_TUPLE))
			{
				Patn_Tuple *patn = (Patn_Tuple *)cast->arms[i].patn;
				for (size_t j = 0; j < tuple->len; ++j)
				{
					if (patn->items[j]->kind == PATN_LIT)
					{
						match = consts[j] &&
								lit_equal(MAC_CAST(Patn_Lit *, patn->items[j])->value,
													MAC_CAST(Expr_Lit *, tuple->items[j])->value);
					}
...
static MAC_HOT void
folder_expr_let(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
	Expr_Let *cast = (Expr_Let *)*node;
	folder_expr(fold, &cast->value, def);

	if (folder_lit(cast->expr))
	{
		*node = cast->expr;
	}
	...
	else if (folder_lit(cast->value) || cast->value->kind == EXPR_IDENT)
	{
		Expr_Node *value;
...
static MAC_HOT void
folder_expr_fun(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
...
static MAC_HOT void
folder_expr_lambda(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
...
static MAC_HOT void
folder_expr_app(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
	...
	{
		// NOTE: Should intrinsic operation be in another pass?
		Sized_Str fun = MAC_CAST(Expr_Ident *, cast->fun)->name;
		if (cast->len == 2 && folder_lit(cast->args[0]) && folder_lit(cast->args[1]))
		{
			if (fun.len > 10 && !memcmp(fun.str, "Intrinsic.", 10))
			{
				Expr_Lit *rhs = (Expr_Lit *)cast->args[0];
				Expr_Lit *lhs = (Expr_Lit *)cast->args[1];

				fun = STR_WLEN(fun.str + 10, fun.len - 10);
				if (STR_EQUAL(fun, STR_STATIC("add")))
				{
					rhs->value.v_int += lhs->value.v_int;
					*node = cast->args[0];
				}
				...
				{
					rhs->value.v_float += lhs->value.v_float;
					*node = cast->args[0];
				}
		...
		else if (cast->len == 1 && folder_lit(cast->args[0]))
		{
			if (fun.len > 10 && !memcmp(fun.str, "Intrinsic.", 10))
			{
				Expr_Lit *rhs = (Expr_Lit *)cast->args[0];
				...
				{
					rhs->value.v_int = -rhs->value.v_int;
					*node = cast->args[0];
				}
				else if (STR_EQUAL(fun, STR_STATIC("fneg")))
				{
					rhs->value.v_float = -rhs->value.v_float;
					*node = cast->args[0];
				}
				else if (STR_EQUAL(fun, STR_STATIC("not")))
				{
					rhs->value.v_bool = !rhs->value.v_bool;
					*node = cast->args[0];
				}
			}
		}
...
static MAC_HOT void
folder_expr_ident(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
	...
	if (expr_tab_get(def, cast->name, &value))
	{
		if (folder_lit(value))
		{
			*node = folder_copy(fold, MAC_CAST(Expr_Lit *, value));
...
static MAC_HOT void
folder_expr_cons(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
	Expr_Constr *cast = (Expr_Constr *)*node;
...
static MAC_HOT void
folder_expr_tuple(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
...
folder_expr(Folder *fold, Expr_Node **node, Expr_Tab *def)
{
	...
	{
		case EXPR_IF:
			folder_expr_if(fold, node, def);
			break;

		case EXPR_MATCH:
			folder_expr_match(fold, node, def);
			break;

		case EXPR_LET:
			folder_expr_let(fold, node, def);
			break;

		case EXPR_FUN:
			folder_expr_fun(fold, node, def);
			break;

		case EXPR_LAMBDA:
			folder_expr_lambda(fold, node, def);
			break;

		case EXPR_APP:
			folder_expr_app(fold, node, def);
			break;

		case EXPR_IDENT:
			folder_expr_ident(fold, node, def);
			break;

		case EXPR_CONSTR:
			folder_expr_cons(fold, node, def);
			break;

		case EXPR_TUPLE:
			folder_expr_tuple(fold, node, def);
			break;
...
folder_decl(Folder *fold, Decl_Node *node, Expr_Tab *def)
{
	switch (node->kind)
	{
		case DECL_LET:
			...
			if (folder_lit(MAC_CAST(Decl_Let *, node)->value) ||
