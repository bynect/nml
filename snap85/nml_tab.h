...
#define TAB_DECL(name, type, key, value) 												\
	...
	type ## _Tab_Key name ## _key(const type ## _Tab *tab, const key k);				\
																						\
	void name ## _copy(const type ## _Tab *MAC_RESTRICT tab,							\
						type ## _Tab *MAC_RESTRICT dest);								\
																						\
	void name ## _union(const type ## _Tab *MAC_RESTRICT tab,							\
						type ## _Tab *MAC_RESTRICT dest);								\
																						\
	void name ## _union_key(const type ## _Tab *MAC_RESTRICT tab,						\
							type ## _Tab *MAC_RESTRICT dest);							\
																						\
	...
	MAC_HOT bool name ## _remove_key(type ## _Tab *tab, type ## _Tab_Key hk);			\
																						\
	MAC_HOT bool name ## _pop(type ## _Tab *tab, key k, value *v);						\
																						\
	MAC_HOT bool name ## _pop_key(type ## _Tab *tab, type ## _Tab_Key hk, value *v);	\
																						\
...
TAB_DECL(id_tab, Id, Sized_Str, uint32_t)
