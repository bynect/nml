...
static MAC_INLINE inline void parser_patn(Parser *parse, Ast_Node **node);
...
static MAC_HOT void
parser_patn_base(Parser *parse, Ast_Node **node)
{
	if (parser_match(parse, TOK_LPAR))
	{
	...
	else if (parser_match(parse, TOK_ULINE) || parser_match(parse, TOK_IDENT))
	{
		*node = ast_ident_new(parse->arena, parse->prev.str);
		(*(Ast_Ident **)node)->patn = true;
	}
	else if ((parse->curr.kind >= TOK_IDENT && parse->curr.kind <= TOK_CHAR) ||
			parser_check(parse, TOK_TRUE) || parser_check(parse, TOK_FALSE))
	{
		parser_eat(parse);
		parser_expr_const(parse, node);
	}
	else
	{
		parser_error(parse, STR_STATIC("Invalid pattern"));
	}
}

static MAC_HOT void
parser_patn_tuple(Parser *parse, Ast_Node **node)
{
	...
	if (parser_match(parse, TOK_COMMA))
	{
		...
		arena_unlock(parse->arena, items);
		*node = ast_tuple_new(parse->arena, items, i);
	}
}

static MAC_INLINE inline void
parser_patn(Parser *parse, Ast_Node **node)
{
...
parser_expr_match(Parser *parse, Ast_Node **node)
{
	...
	size_t len = 8;
	Ast_Node *(*arms)[2] = arena_lock(parse->arena, len * sizeof(Ast_Node *(*)[2]));
	size_t i = 0;
	...
	do
	{
		...
		{
			len += 8;
			arms = arena_update(parse->arena, arms, len * sizeof(Ast_Node *(*)[2]));
		}
		parser_patn(parse, &arms[i][0]);
		parser_expect(parse, TOK_ARROW, STR_STATIC("Expected `->` after pattern"));

		parser_expr(parse, &arms[i][1]);
		++i;
	}
	...
	parse->scope = prev;
	*node = ast_match_new(parse->arena, value, arms, i);
}
...
static MAC_INLINE inline void
parser_const_int(Parser *parse, Ast_Node **node)
{
	int64_t res;
...
static MAC_INLINE inline void
parser_const_float(Parser *parse, Ast_Node **node)
{
...
static MAC_HOT void
parser_const_chars(Parser *parse, Ast_Node **node)
{
...
parser_expr_const(Parser *parse, Ast_Node **node)
{
	...
	{
		case TOK_INT:
			parser_const_int(parse, node);
			break;

		case TOK_FLOAT:
			parser_const_float(parse, node);
			break;

		case TOK_STR:
		case TOK_CHAR:
			parser_const_chars(parse, node);
			break;
