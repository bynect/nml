...
#define TAB_SCHEMEDUMP(value)													\
	...
	{																			\
		...
		{																		\
			...
			{																	\
				fprintf(stdout, "%.*s, ", (uint32_t)value.vars[i].len,			\
						value.vars[i].str);										\
			}																	\
			fprintf(stdout, "%.*s . ", (uint32_t)value.vars[value.len - 1].len,	\
					value.vars[value.len - 1].str);								\
		}																		\
		dbg_dump_type(value.type);												\
	}																			\
	while (false)
...
TAB_IMPL(subst_tab, Subst, Sized_Str, Type,
		TAB_STRDATA, TAB_STRLEN, TAB_STRCMP,
...
TAB_IMPL(scheme_tab, Scheme, Sized_Str, Type_Scheme,
		TAB_STRDATA, TAB_STRLEN, TAB_STRCMP,
		TAB_STRDUMP, TAB_SCHEMEDUMP,
		TAB_ALLOC, 16, 2, TAB_LOAD)
