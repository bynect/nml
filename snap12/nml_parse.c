...
static MAC_HOT void parser_expr_group(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_semi(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_uline(Parser *parse, Ast_Node **node);

static MAC_HOT void parser_expr_if(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_match(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_let(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_fun(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_app(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_ident(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_unary(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_binary(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_tuple(Parser *parse, Ast_Node **node);
static MAC_HOT void parser_expr_const(Parser *parse, Ast_Node **node);

static MAC_HOT void parser_expr_prec(Parser *parse, Ast_Node **node, Parser_Prec prec);
...
static MAC_HOT void
parser_expr_group(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	if (parser_match(parse, TOK_RPAR))
	{
		*node = ast_const_new(parse->arena, loc, const_new(CONST_UNIT, NULL));
		return;
	}
	else if (parse->curr.kind >= TOK_PLUS && parse->curr.kind <= TOK_TILDE)
	{
		...
		if (parser_match(parse, TOK_RPAR))
		{
			*node = ast_ident_new(parse->arena, loc, ident);
			return;
		}
		parser_back(parse);
	}
...
static MAC_HOT void
parser_expr_semi(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	Ast_Node *expr;
	parser_expr(parse, &expr);
	*node = ast_let_new(parse->arena, loc, STR_STATIC("_"), TYPE_NONE, *node, expr);
}

static MAC_HOT void
parser_expr_uline(Parser *parse, Ast_Node **node)
{
	MAC_UNUSED(node);
...
static MAC_HOT void
parser_expr_if(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	Parser_Scope prev = parse->scope;
	parse->scope = SCOPE_LOCAL;
	...
	parse->scope = prev;
	*node = ast_if_new(parse->arena, loc, cond, b_then, b_else);
}

static MAC_HOT void
parser_expr_match(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	Parser_Scope prev = parse->scope;
	parse->scope = SCOPE_LOCAL;
	...
	size_t len = 8;
	struct Ast_Match_Arm *arms = arena_lock(parse->arena, len * sizeof(struct Ast_Match_Arm));
	size_t i = 0;
	...
	do
	{
		...
		{
			len += 8;
			arms = arena_update(parse->arena, arms, len * sizeof(struct Ast_Match_Arm));
		}
	...
	arms = arena_update(parse->arena, arms, i * sizeof(struct Ast_Match_Arm));
	arena_unlock(parse->arena, arms);

	parse->scope = prev;
	*node = ast_match_new(parse->arena, loc, value, arms, i);
}

static MAC_HOT void
parser_expr_let(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	Sized_Str name = parse->curr.str;
	...
	parse->scope = prev;
	*node = ast_let_new(parse->arena, loc, name, hint, value, expr);
}

static MAC_HOT void
parser_expr_fun(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	size_t len = 8;
	...
	parse->scope = prev;
	*node = ast_fun_new(parse->arena, loc, name, hint, pars, len, body, expr);
}

static MAC_HOT void
parser_expr_app(Parser *parse, Ast_Node **node)
{
	if (parser_expr_literal(parse))
	{
		...
		*node = ast_app_new(parse->arena, (*node)->loc, *node, args, i);
	}
}

static MAC_INLINE inline void
parser_expr_ident(Parser *parse, Ast_Node **node)
{
	*node = ast_ident_new(parse->arena, parse->prev.loc, parse->prev.str);
}

static MAC_HOT void
parser_expr_unary(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	...
	*node = ast_unary_new(parse->arena, loc, op, lhs);
}

static MAC_HOT void
parser_expr_binary(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	Token_Kind op = parse->prev.kind;
	...
	*node = ast_binary_new(parse->arena, loc, op, *node, lhs);
}

static MAC_HOT void
parser_expr_tuple(Parser *parse, Ast_Node **node)
{
	...
	else
	{
		...
		*node = ast_tuple_new(parse->arena, items[0]->loc, items, 2);
	}
...
static MAC_HOT void
parser_expr_const(Parser *parse, Ast_Node **node)
{
	Location loc = parse->prev.loc;
	Const_Value value;
	parser_const(parse, &value);
	*node = ast_const_new(parse->arena, loc, value);
}

static MAC_HOT void
parser_expr_prec(Parser *parse, Ast_Node **node, Parser_Prec prec)
{
	parser_eat(parse);
